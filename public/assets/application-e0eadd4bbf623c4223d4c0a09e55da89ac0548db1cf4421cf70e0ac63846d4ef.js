...
gimp_sounds = [new Audio('/audios/gimperSound2.wav'), new Audio('/audios/gimperSound3.wav'), new Audio('/audios/gimperSound4.wav'), new Audio('/audios/gimperSound5.wav')];
...
function link_to_arrange(gimp_list) {
  if (gimp_list.length > 0) {
    var element_list = [];
    for (var i = 0; i < gimp_list.length; i++) {
      element_list.push('gimps[]=' + gimp_list[i]);
    }
    var element_str = element_list.join('&');
...
function gimp_on_up(gimp_num) {
  var lil_g = document.getElementById('lil_gimper_' + gimp_num);
  var gimp_sound = gimp_sounds[Math.floor(Math.random() * gimp_sounds.length)];
...
function generate_img() {
  var element = document.getElementById('arrangement_canvas');
  var gimpers = element.getElementsByClassName('arrangeable_gimper');
  var elCanvasso = document.createElement('canvas');
  elCanvasso.width = element.clientWidth;
  elCanvasso.height = element.clientHeight;
  var ctx = elCanvasso.getContext('2d');
  for (var i = 0; i < gimpers.length; i++) {
    var gimpdiv = gimpers[i];
    var gimpsvg = gimpdiv.getElementsByTagName("img")[0];
    var x = parseInt(gimpdiv.style.left);
    var y = parseInt(gimpdiv.style.top);
    ctx.drawImage(gimpsvg, x, y);
  }
  var flower_img = elCanvasso.toDataURL("image/png");
  return flower_img;
  // temp code
